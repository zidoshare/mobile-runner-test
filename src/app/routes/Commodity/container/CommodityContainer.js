...
class CommodityContainer extends Component {
  ...
  getDom = () => {
    if (this.state.isEnd) {
      return [<ListItem key="to-my-auction" align="middle" multipleLine arrow="horizontal" onClick={() => {
        this.props.history.push('/myAuction')
      }}>
        竞拍结束<Brief>点击查看我的拍卖</Brief>
      </ListItem>]
    } else {
      const { getFieldProps } = this.props.form
      const { commodity, subLoading } = this.props.com
      return [<InputItem key="input-add" {...getFieldProps('money', {
        rules: [{
          required: true, message: '请输入加价金额'
        }, {
          validator(rule, value, callback) {
            const result = Number(value[0])
            if (result >= commodity.addFloorPrice) {
              callback([])
            }
            callback([new Error('低于最低加价金额')])
          }
        }],
        normalize: (v, prev) => {
        ...
        clear
      >加价金额</InputItem>, <ListItem key="btn-add">
        <Button loading={subLoading} type="primary" onClick={this.handleSubmit}>加价竞拍</Button>
      </ListItem>, <Picker key="input-add-list" data={commodity.basePrice.split(',').map(value => ({
        value: value,
        label: '加价' + value + '元',
  ...
  render() {
    const { commodity, loading, infoLoading, info, detailLoading, detail, isFollow, updateTime } = this.props.com
    return (
          ...
          <div className="commodity-content-wrapper">
            ...
            <div className="com-op-container">
              <Flex className="title-line">
                ...
                <FlexItem style={{ fontSize: 12, whiteSpace: 'nowrap' }}>距结束：<Timer endTime={commodity.endTime} callback={this.handleToEnd} /></FlexItem>
